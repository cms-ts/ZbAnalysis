...
if [ -d $WORKDIR/$VERSION/$JOBDIR ]; then
  ...
  echo 'Preparing '$WORKDIR/$VERSION/MuEG_2012_merge.root
  rm -f $WORKDIR/$VERSION/MuEG_2012_merge.root
  hadd $opts $WORKDIR/$VERSION/MuEG_2012_merge.root $WORKDIR/$VERSION/MuEG_2012*.root
else
