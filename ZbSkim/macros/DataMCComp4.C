...
void DataMCComp4(string& title="", int plot=0, int ilepton=1) {
	...
	if (ilepton==1) {
	  mc1->cd("demoEleGen");
	  TH1F* h_mc_truth = (TH1F*)gDirectory->Get(title.c_str());
	  mc1->cd("demoEle");
	  TH1F* h_mc_reco  = (TH1F*)gDirectory->Get(title.c_str());
	  mc2->cd("demoEleDump");
	  TH2F* h_mc_mtx  = (TH2F*)gDirectory->Get(title.c_str());
	}
	if (ilepton==2) {
	  mc1->cd("demoMuoGen");
	  TH1F* h_mc_truth = (TH1F*)gDirectory->Get(title.c_str());
	  ...
	  TH2F* h_mc_mtx  = (TH2F*)gDirectory->Get(title.c_str());
        ...
        RooUnfoldResponse response (h_mc_reco, h_mc_truth, h_mc_mtx);
...
// default n=1000, cambia di molto la velocita' dell'algo
	RooUnfoldSvd unfold (&response, h_mc_reco, 7, 100);
	TH1F* h_mc_unf= (TH1F*) unfold.Hreco();
       
	double N = (h_mc_truth->Integral())/(h_mc_unf->Integral());
        h_mc_unf->Scale(N);

	TCanvas* c1 = new TCanvas("c", "c", 800, 600);
	c1->cd();
	h_mc_unf->Draw();
	h_mc_truth->Draw("SAME");
	h_mc_reco->Draw("SAME");
	h_mc_unf->SetLineColor(kBlack);
