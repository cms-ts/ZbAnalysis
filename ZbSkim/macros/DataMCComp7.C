...
if (numB==2) {
  ...
  drawInclusive =0;
	...
	if (ilepton==1) {
	  ...
	  in5.open((path + "/electrons/" + version + "/" + subdir + "/xsecs_unfolding" + dirbSel + "/" + "w_delta_phi_ee" + "_xsecs_unfolding.dat").c_str());
	}
	if (ilepton==2) {
	  ...
	  in5.open((path + "/muons/" + version + "/" + subdir + "/xsecs_unfolding" + dirbSel + "/" + "w_delta_phi_mm" + "_xsecs_unfolding.dat").c_str());
	}
	in1 >> sum1; in1 >> sum1_b;
	in2 >> sum2; in2 >> sum2_b;
	in3 >> sum3; in3 >> sum3_b;
	...
	if (plot) {
	  ...
	  if (isratio==0) {
	    ...
	    out << endl;
	    out << std::setw(27) << "data";
	    out << std::setw(13) << "bkg";
	    out << std::setw(13) << "eff";
	    out << std::setw(13) << "jec";
	    out << std::setw(13) << "jer";
	    out << std::setw(13) << "pu";
	    if (useSysDR) out << std::setw(13) << "DR";
	    out << std::setw(13) << "bkg";
	    out << std::setw(13) << "ttbar";
	    out << std::setw(13) << "bfit";
	    if (useSysBfit2) out << std::setw(13) << "btemp";
	    out << std::setw(13) << "btag";
	    out << std::setw(13) << "unfold";
	    out << std::setw(13) << "unfold";
	    out << std::setw(13) << "lumi";
	    if (useSysRMS) out << std::setw(13) << "unfold";
	    out << std::setw(13) << "total";
	    out << std::setw(13) << "total";
	    out << std::setw(13) << "total";
	    out << endl;
	    out << std::setw(27) << "stat";
	    out << std::setw(13) << "stat";
	    out << std::setw(13) << "syst";
	    out << std::setw(13) << "syst";
	    out << std::setw(13) << "syst";
	    out << std::setw(13) << "syst";
	    if (useSysDR) out << std::setw(13) << "syst";
	    out << std::setw(13) << "syst";
	    out << std::setw(13) << "stat";
	    out << std::setw(13) << "stat";
	    if (useSysBfit2) out << std::setw(13) << "syst";
	    out << std::setw(13) << "syst";
	    out << std::setw(13) << "stat";
	    out << std::setw(13) << "syst";
	    out << std::setw(13) << "syst";
	    if (useSysRMS) out << std::setw(13) << "rms";
	    out << std::setw(13) << "stat";
	    out << std::setw(13) << "syst";
	    out << std::setw(13) << "error";
	    out << std::setw(8) << "%";
	    out << endl;
	    for (int i=0;i<=h_data->GetNbinsX()+1;i++) {
	      out << std::fixed;
	      ...
	      out << " +- ";
	      out << std::setw(9) << stat_bkg->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << syst_eff->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << syst_jec->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << syst_jer->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << syst_pu->GetBinError(i);
	      if (useSysDR) {
	        out << " +- ";
	        out << std::setw(9) << syst_dr->GetBinError(i);
	      }
	      out << " +- ";
	      out << std::setw(9) << syst_bkg->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << stat_top->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << stat_bfit->GetBinError(i);
	      if (useSysBfit2) {
	        out << " +- ";
	        out << std::setw(9) << syst_bfit2->GetBinError(i);
	      }
	      out << " +- ";
	      out << std::setw(9) << syst_btag->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << stat_unfold->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << syst_unfold->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << syst_lumi->GetBinError(i);
	      if (useSysRMS) {
	        out << " +- ";
	        out << std::setw(9) << h_data->GetBinContent(i)*(rms/tot);
	      }
	      out << " => ";
	      out << std::setw(9) << h_data_stat->GetBinError(i);
	      out << " +- ";
	      out << std::setw(9) << h_data_syst->GetBinError(i);
	      out << " => ";
	      out << std::setw(9) << h_data_tot->GetBinError(i);
	      out << " => ";
	  ...
	  for (int i=0;i<=h_data_b->GetNbinsX()+1;i++) {
	    ...
	    out << " +- ";
	    out << std::setw(9) << stat_b_bkg->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << syst_b_eff->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << syst_b_jec->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << syst_b_jer->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << syst_b_pu->GetBinError(i);
	    if (useSysDR) {
	      out << " +- ";
	      out << std::setw(9) << syst_b_dr->GetBinError(i);
	    }
	    out << " +- ";
	    out << std::setw(9) << syst_b_bkg->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << stat_b_top->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << stat_b_bfit->GetBinError(i);
	    if (useSysBfit2) {
	      out << " +- ";
	      out << std::setw(9) << syst_b_bfit2->GetBinError(i);
	    }
	    out << " +- ";
	    out << std::setw(9) << syst_b_btag->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << stat_b_unfold->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << syst_b_unfold->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << syst_b_lumi->GetBinError(i);
	    if (useSysRMS) {
	      out << " +- ";
	      out << std::setw(9) << h_data_b->GetBinContent(i)*(rms_b/tot_b);
	    }
	    out << " => ";
	    out << std::setw(9) << h_data_b_stat->GetBinError(i);
	    out << " +- ";
	    out << std::setw(9) << h_data_b_syst->GetBinError(i);
	    out << " => ";
	    out << std::setw(9) << h_data_b_tot->GetBinError(i);
	    out << " => ";
	    out << std::setprecision(1);
