...
if [ $n -eq 2 ]; then
  root -l -q -b DataMCComp6.C+\($d,\"w_second_bjet_pt\",1,0,$n\)
  root -l -q -b DataMCComp6.C+\($d,\"w_second_bjet_eta\",1,0,$n\)
  root -l -q -b DataMCComp6.C+\($d,\"w_second_bjet_eta_abs\",1,0,$n\)
fi
...
if [ $n -ne 0 ]; then
  root -l -q -b DataMCComp6.C+\($d,\"w_pt_Z_b\",1,0,$n\)
