...
//cout << endl;
	...
	for (int i=0;i<=h_data_stat->GetNbinsX()+1;i++) {
	  ...
	  if (x[0][0][i]*x[1][0][i] != 0) {
	    val = (x[0][0][i]/(x[0][N-4][i]**2+x[0][2][i]**2+x[0][3][i]**2)+x[1][0][i]/(x[1][N-4][i]**2+x[1][2][i]**2+x[1][3][i]**2))/(1./(x[0][N-4][i]**2+x[0][2][i]**2+x[0][3][i]**2)+1./(x[1][N-4][i]**2+x[1][2][i]**2+x[1][3][i]**2));
	    h_data_stat->SetBinContent(i, val);
	    val = TMath::Sqrt(1./(1./(x[0][N-4][i]**2)+1./(x[1][N-4][i]**2)));
	    h_data_stat->SetBinError(i, val);
	  }
	  val = (TMath::Sqrt(x[0][N-3][i]**2-x[0][2][i]**2-x[0][3][i]**2)+TMath::Sqrt(x[1][N-3][i]**2-x[1][2][i]**2-x[1][3][i]**2))/2.;
	  val = TMath::Sqrt(val**2+(1./(1./(x[0][2][i]**2+x[0][3][i]**2)+1./(x[1][2][i]**2+x[1][3][i]**2))));
	...
	for (int i=0;i<=h_data_b_stat->GetNbinsX()+1;i++) {
	  ...
	  if (x_b[0][0][i]*x_b[1][0][i] != 0) {
	    val = (x_b[0][0][i]/(x_b[0][N-4][i]**2+x_b[0][2][i]**2+x_b[0][3][i]**2)+x_b[1][0][i]/(x_b[1][N-4][i]**2+x_b[1][2][i]**2+x_b[1][3][i]**2))/(1./(x_b[0][N-4][i]**2+x_b[0][2][i]**2+x_b[0][3][i]**2)+1./(x_b[1][N-4][i]**2+x_b[1][2][i]**2+x_b[1][3][i]**2));
	    h_data_b_stat->SetBinContent(i, val);
	    val = TMath::Sqrt(1./(1./(x_b[0][N-4][i]**2)+1./(x_b[1][N-4][i]**2)));
	    h_data_b_stat->SetBinError(i, val);
	  }
	  val = (TMath::Sqrt(x_b[0][N-3][i]**2-x_b[0][2][i]**2-x_b[0][3][i]**2)+TMath::Sqrt(x_b[1][N-3][i]**2-x_b[1][2][i]**2-x_b[1][3][i]**2))/2.;
	  val = TMath::Sqrt(val**2+(1./(1./(x_b[0][2][i]**2+x_b[0][3][i]**2)+1./(x_b[1][2][i]**2+x_b[1][3][i]**2))));
